...
bool Shot::operator==( const Shot & dane) const{
    if(player_id==dane.player_id && x_start==dane.x_start && y_start==dane.y_start && direction==dane.direction && (flight_periods-dane.flight_periods)<=SPEEDSHOT)    return true;
    else return false;
}
